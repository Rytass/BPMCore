...
export class TemplateService {
  ...
  async listApprovalTemplates(): Promise<readonly ApprovalTemplateEntity[]> {
  ...
  async updateApprovalTemplateDraft(
    ...
    if (existing.status !== ApprovalTemplateVersionStatusEnum.DRAFT) {
      throw new ConflictException('Only draft approval template versions can be updated');
    }
  ...
  async forkApprovalTemplate(
    ...
    if (existingDraft) {
      throw new ConflictException('A draft approval template version already exists');
    }
    ...
    return this.templateVersionRepository.save(
      this.templateVersionRepository.create({
        ...
        templateId: template.id,
        version: nextVersion,
        workflowDefinition: source?.workflowDefinition ?? EMPTY_WORKFLOW_DEFINITION,
      }),
    );
  }
  ...
  async publishApprovalTemplateVersion(
    ...
    if (version.status !== ApprovalTemplateVersionStatusEnum.DRAFT) {
      throw new ConflictException('Only draft approval template versions can be published');
    }
    ...
    return this.templateRepository.manager.transaction(
      async (manager): Promise<ApprovalTemplateVersionEntity> => {
        ...
        if (!template) {
          throw new NotFoundException(`Approval template ${version.templateId} was not found`);
        }
  ...
  async rollbackApprovalTemplateVersion(
    ...
    if (target.status === ApprovalTemplateVersionStatusEnum.DRAFT) {
      throw new ConflictException('Draft approval template versions cannot be rollback targets');
    }
    ...
    return this.templateRepository.manager.transaction(
      async (manager): Promise<ApprovalTemplateVersionEntity> => {
        ...
        if (!template) {
          throw new NotFoundException(`Approval template ${target.templateId} was not found`);
        }

        if (template.currentVersionId && template.currentVersionId !== target.id) {
          await versionRepository.update(
            { id: template.currentVersionId },
            {
              archivedAt: new Date(),
  ...
  private async validatePublishableVersion(
    ...
    if (!version.formDefinitionVersionId) {
      throw new BadRequestException('Approval template version must bind a form definition version before publishing');
    }
    ...
    if (formVersion.status !== FormDefinitionVersionStatusEnum.PUBLISHED) {
      throw new BadRequestException('Approval template can only bind a published form definition version');
    }
    ...
    const conditionErrors = this.conditionService.lintExpressions(
      readConditionExpressions(version.workflowDefinition, version.initiatorPolicyCel),
    );
  ...
  private async getTemplateOrThrow(id: string): Promise<ApprovalTemplateEntity> {
  ...
  private async getTemplateVersionOrThrow(
    ...
    if (!entity) {
      throw new NotFoundException(`Approval template version ${id} was not found`);
    }
...
): readonly { readonly expression: string | null | undefined; readonly label: string }[] {
  return [
