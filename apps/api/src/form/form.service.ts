...
export class FormService {
  ...
  async createFormDefinition(
    ...
    const schemas = this.parseSchemasOrThrow(input.schemaJson, input.uiSchemaJson);
  ...
  async listFormDefinitions(): Promise<readonly FormDefinitionEntity[]> {
  ...
  async updateFormDefinitionDraft(
    ...
    const existing = await this.getFormDefinitionVersionOrThrow(input.versionId);

    if (existing.status !== FormDefinitionVersionStatusEnum.DRAFT) {
      throw new ConflictException('Only draft form definition versions can be updated');
    }
  ...
  async forkFormDefinition(
    ...
    if (existingDraft) {
      throw new ConflictException('A draft form definition version already exists');
    }
  ...
  async publishFormDefinitionVersion(
    ...
    if (version.status !== FormDefinitionVersionStatusEnum.DRAFT) {
      throw new ConflictException('Only draft form definition versions can be published');
    }

    this.parseSchemasOrThrow(JSON.stringify(version.schema), JSON.stringify(version.uiSchema));
  ...
  async rollbackFormDefinitionVersion(
    ...
    if (target.status === FormDefinitionVersionStatusEnum.DRAFT) {
      throw new ConflictException('Draft form definition versions cannot be rollback targets');
    }
    ...
    return this.formDefinitionRepository.manager.transaction(
      async (manager): Promise<FormDefinitionVersionEntity> => {
        ...
        if (definition.currentVersionId && definition.currentVersionId !== target.id) {
          await versionRepository.update(
            { id: definition.currentVersionId },
            {
              archivedAt: new Date(),
  ...
  private async getFormDefinitionVersionOrThrow(
    ...
    if (!entity) {
      throw new NotFoundException(`Form definition version ${id} was not found`);
    }

    return entity;
  }

  private async readNextVersionNumber(formDefinitionId: string): Promise<number> {
