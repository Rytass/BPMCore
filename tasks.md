...
- 改完某欄型別後，下方「欄設定」面板仍停在別欄；應自動選取剛改的欄。
- 剛改成下拉選單／自動完成的欄沒有「尚未設定選項」提示，要到發布才被擋。
- 欄清單缺完成度訊號（選項空、參數未綁、key 仍是 `text_1`），只能逐欄點開檢查。
- 「設定此欄」只有 icon，說明文字只在**沒選任何欄時**出現——需要它時它不在。
...
- 欄清單欄序改為 標題 → 型別 → 必填 → 狀態；`欄位 Key` 降為次要樣式並說明用途
  （流程條件與 API）。
- 標題輸入時自動產生 key（可覆寫）：現在打完中文標題，key 永遠是 `text_1`。
- 欄設定改為就地展開或右側抽屜，讓「選了哪一欄」與「在設定什麼」在視覺上相連。
- 型別欄加寬（「下拉選單」被截斷）；移除 toggle 與表頭重複的「必填」字樣。
