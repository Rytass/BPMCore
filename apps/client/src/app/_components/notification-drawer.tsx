...
export function NotificationDrawer(): ReactElement | null {
  ...
  const loadPage = useCallback(
    async (nextPage: number, append: boolean): Promise<void> => {
      ...
      try {
        ...
        setRows((currentRows): readonly NotificationRecord[] =>
          append
            ? [...currentRows, ...result.notifications]
  ...
  const handleFilterChange = useCallback(
    (event: ChangeEvent<HTMLInputElement>): void => {
      ...
      if (nextValue === 'all' || nextValue === 'read' || nextValue === 'unread') {
        setFilter(nextValue);
      }
    },
    [],
  ...
  const handleMarkAllRead = useCallback(async (): Promise<void> => {
    if (!currentMemberId || bulkLoading) {
      return;
    }
  ...
  const handleOpenInstance = useCallback(
    async (record: NotificationRecord): Promise<void> => {
      if (!record.instanceId || !currentMemberId) {
        return;
      }
  ...
  const groupedRows = useMemo(
    (): ReadonlyArray<readonly [TimeGroup, readonly NotificationRecord[]]> => {
      const now = new Date();
      const buckets = TIME_GROUP_ORDER.reduce<Record<TimeGroup, NotificationRecord[]>>(
        (accumulator, group) => {
          accumulator[group] = [];
