...
Last verified against (2026-09-15, pending the release after `v0.13.3`): `libs/shared@0.13.3`, `libs/bpm-core-client@0.13.3`, `libs/bpm-core@0.13.3` (`@rytass/bpm-core-nestjs-module`), `libs/bpm-core-react@0.13.3`. All four packages are one fixed version set, so these numbers move together. The table field work (ADR 16, P0–P4) shipped in `v0.12.0`. The builder usability round that followed P4 (see `tasks.md`) changed no export, so this inventory is unchanged by it. The upstream-integration round below shipped in `v0.13.0`. `v0.13.1` changed the release pipeline only. The org unit code fix (BPM-10), the NOTIFY webhook shared contract (ADR 18 P0) the endpoint registry/catalog (ADR 18 P1), the delivery outbox (ADR 18 P2), delivery observability (ADR 18 P3) and the designer webhook panel with its shared catalog lint (ADR 18 P4) are inventoried here but not yet released; `nx release` sets the next number at publish time.
...
| `NotifyWebhookEndpointContract` / `NotifyWebhookCatalogIssue` / `NotifyWebhookCatalogIssueCode` | type/interface | The descriptor subset the catalog lint reads, and a catalog/form publish problem as a code (`ENDPOINT_MISSING`, `ENDPOINT_DEPRECATED`, `PARAMETER_REQUIRED`, `PARAMETER_UNKNOWN`, `FIELD_MISSING`, `FIELD_INCOMPATIBLE`, `CONSTANT_REQUIRED_NULL`, `CONSTANT_INCOMPATIBLE`, `CONTEXT_INCOMPATIBLE`) |
